...
?>
	<div id="container">
		<div id="activeFriends">
			...
			<?php
			if(!isset($error)){
				$result = @$connect->query("SELECT Users.Id, Users.FirstName, Users.LastName, Users.Color, Users.Logo FROM Users CROSS JOIN Relations WHERE (Users.Id = Relations.FirstPerson AND Relations.Status = 'accept' AND Relations.SecondPerson = '$id') OR (Users.Id = Relations.SecondPerson AND Relations.Status = 'accept' AND Relations.FirstPerson = '$id') ORDER BY Users.FirstName");
				while($person = mysqli_fetch_array($result)){
			?>
			<div class="personAccept">
				<img <?php echo 'style="background-color: rgb(' . $person['Color'] . ');" src="../image/avatar' . $person['Logo'] . '.png"'; ?> alt="" />
		...
		<div id="notYet">
			<div id="waitingForAccept">
				...
				<?php
				if(!isset($error)){
					$result = @$connect->query("SELECT Users.FirstName, Users.LastName, Users.Color, Users.Logo, Relations.Id FROM Users INNER JOIN Relations ON Users.Id = Relations.FirstPerson WHERE Relations.SecondPerson = '$id' AND Relations.Status = 'wait'");
					while($person = mysqli_fetch_array($result)){
				?>
				<div class="person">
					<img <?php echo 'style="background-color: rgb(' . $person['Color'] . ');" src="../image/avatar' . $person['Logo'] . '.png"'; ?> alt="" />
			...
			<div id="sentInvitations">
				...
				<?php
				if(!isset($error)){
					$result = @$connect->query("SELECT Users.FirstName, Users.LastName, Users.Color, Users.Logo, Relations.Id FROM Users INNER JOIN Relations ON Users.Id = Relations.SecondPerson WHERE Relations.FirstPerson = '$id' AND Relations.Status = 'wait'");
					while($person = mysqli_fetch_array($result)){
				?>
				<div class="person">
					<img <?php echo 'style="background-color: rgb(' . $person['Color'] . ');" src="../image/avatar' . $person['Logo'] . '.png"'; ?>  alt="" />
